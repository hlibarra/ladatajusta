...
def signal_handler(signum, frame):
    ...
    print(f"\n[SIGNAL] Received signal {signum}, requesting graceful shutdown...")
...
async def run_scraping():
    ...
    try:
        ...
        log(f"Found {len(sources)} active source(s)")
...
async def main():
    ...
    while not shutdown_requested:
        ...
        if minutes_since_scrape >= SCRAPE_INTERVAL_MINUTES:
            try:
                await run_scraping()
        ...
        if minutes_since_ai >= AI_PROCESS_INTERVAL_MINUTES:
            try:
                await process_ai()
