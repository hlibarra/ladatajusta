"""
Scraper de Página 12 para guardar en la tabla scraping_items
Usa el feed RSS para obtener enlaces y Playwright para extraer contenido
...
# Scraper metadata
SCRAPER_NAME = "pagina12_rss_playwright"
SCRAPER_VERSION = "1.0.0"
SOURCE_MEDIA = "pagina12"
...
def extract_section_from_url(url: str) -> str:
    """
    Extrae la sección de una URL de Página 12
    Las URLs de P12 no tienen sección en el path, retorna None
    """
    return None


async def get_rss_articles() -> list:
    """
    Obtiene los artículos del feed RSS de Página 12
    """
    print(f"[RSS] Fetching RSS feed: {RSS_URL}")

    async with httpx.AsyncClient(timeout=30.0) as client:
        response = await client.get(RSS_URL)
        if response.status_code != 200:
            raise Exception(f"Failed to fetch RSS: HTTP {response.status_code}")
        ...
        print(f"[RSS] Found {len(articles)} articles in RSS feed")
        return articles
...
async def procesar_noticia(context, rss_article: dict):
    """
    ...
    try:
        ...
        # Extraer título (usar del RSS si no se encuentra en la página)
        titulo = rss_article.get('title', '')
        titulo_selectors = [
            "h1.article-title",
            "h1[class*='title']",
            "article h1",
        ...
        for selector in titulo_selectors:
            try:
                if await page.locator(selector).first.is_visible(timeout=1000):
                    page_titulo = await page.locator(selector).first.text_content()
                    if page_titulo and len(page_titulo.strip()) > 10:
                        titulo = page_titulo.strip()
                        break
            except:
                continue
        ...
        subtitulo = rss_article.get('summary', '')
        subtitulo_selectors = [
            ".article-summary",
            ".article-deck",
            "h2.volanta",
        ...
        for selector in subtitulo_selectors:
            try:
                if await page.locator(selector).first.is_visible(timeout=500):
                    page_subtitulo = await page.locator(selector).first.text_content()
                    if page_subtitulo:
                        subtitulo = page_subtitulo.strip()
                        break
            except:
                continue
        ...
        if not contenido or len(contenido) < 100:
            try:
                parrafos = page.locator("article p, .article-body p, .nota-cuerpo p")
        ...
        if not contenido or len(contenido) < 50:
            print(f"[SKIP] No se encontró contenido suficiente: {enlace}")
            await page.close()
            return False

        # Extraer autor (usar del RSS si no se encuentra)
        autor = rss_article.get('author', '')
        autor_selectors = [
            ".article-author",
            "[class*='author']",
            ".nota-autor",
        ...
        for selector in autor_selectors:
            try:
                if await page.locator(selector).first.is_visible(timeout=500):
                    page_autor = await page.locator(selector).first.text_content()
                    if page_autor:
                        autor = page_autor.strip()
                        break
            except:
                continue
        ...
        for selector in seccion_selectors:
            try:
                if await page.locator(selector).first.is_visible(timeout=500):
                    seccion = await page.locator(selector).first.text_content()
                    if seccion:
        ...
        tags = []
        try:
            tag_elements = page.locator("a[href*='/tag/'], .article-tags a, .nota-tags a")
        ...
        data = {
            ...
            'author': autor if autor else None,
            ...
            'status': 'scraped',
            'status_message': 'Successfully scraped from Página 12',
            'extra_metadata': json.dumps({
                'user_agent': await page.evaluate('navigator.userAgent'),
                'viewport': page.viewport_size,
                'rss_title': rss_article.get('title', ''),
                'rss_summary': rss_article.get('summary', ''),
            })
        }
...
async def main():
    ...
    print(f"[START] Iniciando scraper de Página 12")
    ...
    try:
        # Obtener artículos del RSS
        rss_articles = await get_rss_articles()

        if not rss_articles:
            print("[WARN] No se encontraron artículos en el RSS")
            return {
                'status': 'error',
                'items_scraped': 0,
        ...
        async with async_playwright() as p:
            ...
            print(f"[LINKS] {len(rss_articles)} artículos del RSS para procesar")

            # Procesar noticias en lotes (concurrencia controlada)
            for i in range(0, len(rss_articles), CONCURRENCY):
                grupo = rss_articles[i:i + CONCURRENCY]
                print(f"\n[BATCH] Procesando lote {i // CONCURRENCY + 1}/{(len(rss_articles) - 1) // CONCURRENCY + 1}")
                tareas = [procesar_noticia(context, article) for article in grupo]
            ...
            print(f"[INFO] Run ID: {scraping_run_id}")
            print(f"[INFO] Items scrapeados: {items_scraped}/{len(rss_articles)}")
            ...
            return {
                ...
                'total_links': len(rss_articles),
                'duration_seconds': duration,
                'message': f'Scraped {items_scraped} new items out of {len(rss_articles)} RSS articles'
            }
