"""
Scraper de Clarín para guardar en la tabla scraping_items
Usa Playwright para acceder a la página de últimas noticias y extraer contenido
...
# ===== CONFIGURACIÓN =====
URL_ULTIMO_MOMENTO = "https://www.clarin.com/ultimo-momento"
CONCURRENCY = 5
...
# Scraper metadata
SCRAPER_NAME = "clarin_playwright"
...
def extract_section_from_url(url: str) -> str:
    """
    Extrae la sección de una URL de Clarín
    Ejemplo: /politica/titulo-noticia -> politica
    """
    try:
        ...
        if parts:
            section = parts[0]
            # Verificar que no sea una fecha o ID
...
async def procesar_noticia(context, enlace):
    ...
    try:
        ...
        for selector in titulo_selectors:
            try:
                if await page.locator(selector).first.is_visible(timeout=1000):
                    ...
                    if titulo and len(titulo.strip()) > 10:
                        break
            except:
                continue
        ...
        if not contenido or len(contenido) < 100:
            try:
                parrafos = page.locator("article p, .body-nota p, .article-body p")
        ...
        tags = []
        try:
            tag_elements = page.locator("a[href*='/tema/'], .article-tags a, .tags a")
        ...
        data = {
            ...
            'status': 'scraped',
            'status_message': 'Successfully scraped from Clarín',
...
async def main():
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            print(f"[NAV] Navegando a {URL_ULTIMO_MOMENTO}")
            await page.goto(URL_ULTIMO_MOMENTO, timeout=60000, wait_until="domcontentloaded")
            ...
            # Clarín usa enlaces con el patrón /{seccion}/{titulo}.html
            # Excluir páginas de temas (/tema/), autores (/autor/), secciones base
            enlaces = await page.eval_on_selector_all(
                "a[href]",
                """els => els.map(e => e.href)
                    .filter(h => {
                        // Debe ser de Clarín y terminar en .html
                        if (!h.includes('clarin.com') || !h.endsWith('.html')) return false;

                        // Excluir páginas que NO son artículos
                        const excludePatterns = [
                            '/tema/',      // Páginas de etiquetas/temas
                            '/temas/',     // Variante de temas
                            '/autor/',     // Páginas de autores
                            '/rss',        // Feeds RSS
                            '/suscripciones/', // Páginas comerciales
                            '/especiales/',    // Especiales/landing
                            '/newsletters/',   // Newsletters
                            '/servicios/',     // Servicios
                            '/buscar/'         // Búsqueda
                        ];

                        if (excludePatterns.some(p => h.includes(p))) return false;

                        // Debe tener al menos 2 segmentos en el path (seccion/titulo.html)
                        try {
                            const url = new URL(h);
                            const parts = url.pathname.split('/').filter(p => p && p !== '');
                            // Necesitamos al menos seccion + titulo.html
                            if (parts.length < 2) return false;
                            // El último segmento debe ser el artículo (termina en .html)
                            // Y debe tener suficiente contenido (no solo "index.html")
